/* 
  编译
    ...
    colcon build --packages-select pkg05_cpp02_service
...
public:
  MinimalService():Node("minimal_service"){
    RCLCPP_INFO(this->get_logger(),"[MinimalService] 节点初始化");
    // 3-1.创建服务端;

    server_ = this->create_service<AddInts>("add_ints", std::bind(&MinimalService::AddCallback, this, _1, _2));
    RCLCPP_INFO(this->get_logger(),"add_ints 服务端启动完毕，等待请求提交...");
  }

protected:
  void AddCallback(const AddInts::Request::SharedPtr req,const AddInts::Response::SharedPtr res){

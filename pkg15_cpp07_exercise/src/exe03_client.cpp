...
{
public:
  ExeDistanceClient() : Node("my_node_cpp")
  {
    // 3-1.创建客户端；
  ...
  // 3-2.连接服务；
  bool connect_server(){
    while (!distance_client->wait_for_service(std::chrono::seconds(1s))) {
      if (!rclcpp::ok()) {
        RCLCPP_ERROR(this->get_logger(), "客户端退出！");
        return false;
      }
  ...
  rclcpp::Client<base_interfaces_demo::srv::Distance>::FutureAndRequestId send_distance(float x,float y,float theta){
    ...
    auto distance_requst = std::make_shared<base_interfaces_demo::srv::Distance::Request>();
    distance_requst->x = x;
    distance_requst->y = y;
    distance_requst->theta = theta;
    return distance_client->async_send_request(distance_requst);
  }
...
int main(int argc, char *argv[])
{
  ...
  RCLCPP_INFO(client->get_logger(),"传入目标的位姿参数:(x,y,theta)=(%f, %f, %f)",x, y, theta);

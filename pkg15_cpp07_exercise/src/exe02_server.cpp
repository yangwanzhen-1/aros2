...
{
public:
  ExeDistanceClient() : Node("exe_distance_client"),turtle1_x(0.0),turtle1_y(0.0)
  {
    RCLCPP_INFO(this->get_logger(), "节点创建, exe_distance_client");
    // 3-1.创建乌龟姿态订阅方，回调函数中获取x坐标与y坐标；
    pose_sub = this->create_subscription<turtlesim::msg::Pose>("/turtle1/pose", 10, std::bind(&ExeDistanceClient::poseCallBack, this, _1));
    // 3-2.创建服务端；
    distance_server = this->create_service<base_interfaces_demo::srv::Distance>("distance", std::bind(&ExeDistanceClient::distanceCallBack, this, _1, _2));
...
protected:
  ...
  void distanceCallBack(const std::shared_ptr<base_interfaces_demo::srv::Distance::Request> request,
...
int main(int argc, char *argv[])
{
  ...
  rclcpp::spin(std::make_shared<ExeDistanceClient>());
